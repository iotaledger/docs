...
- \`Trinary\` - Alternative to Binary, Trinary is a base-3 numeral system. Read more here
    ...
    - \`Trits\` - Analogous to bits, a ternary digit is a trit (trinary digit). The 
digits may have the values 1, 0, or -1.

    - \`Trytes\` - Analogous to bytes, a tryte consists of 3 trits, which can represent 
27 values. in IOTA, trytes are represented as characters '9,A-Z'.
...
stores all transaction data of the IOTA network. 
    - The Tangle is the first distributed ledger to achieve scalability, 
...
- \`iota.lib.*\` - The IOTA library, it has all the necessary functionality to fully use 
...
private and public keys. Keep it secure and don't share it with anyone. 
    - Seeds in IOTA consist of 81 Trytes ('A-Z,9'), which is equivalent to 384 bits security. 
...
to choose between 3 security models:
