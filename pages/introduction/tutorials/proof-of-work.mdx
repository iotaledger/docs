...
So will we have publish a transaction without using the `attachToTangle` command of the node. This is achieved by overriding the behaviour of the library with a PoW service. There are three to choose from:
...
In this tutorial we will use the public Sandbox on the IOTA Devnet. While the public node has `AttachToTangle` available a PowBox can be easily provisioned to be used on the IOTA Mainnet.
