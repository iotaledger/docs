...
**Transfers:**
<Code>{`const transfers = [ { value: 0, address: trytes, // HELLOWORLDHELLOWORLDHELLO....WORLDD message: message } ]`}
</Code>
...
**Transfer Function:**
<Code>{`iota.sendTransfer(trytes, 3, 9, transfers, (error, success) => { if (error) { console.log(error) } else { console.log(success) } })`}
</Code>
