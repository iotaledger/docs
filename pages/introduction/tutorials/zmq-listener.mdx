...
To allow for **realtime** network listening the IOTA Reference Implimentation (IRI), the most common node software, impliments Zero Message Queue for asyncronous messaging between the Node and the Client. The specification can be found [here](https://github.com/iotaledger/iri/tree/dev/src/main/java/com/iota/iri/zmq).
...
Then we subscribe to different relevant topics. The topics here are:

- `tx` - all transactions
- `sn` - transactions that have transitioned to confirmed

This on an example of the topics a correctly conffigured IRI nodes offers. To see the full info on the subscription channels check the [IRI github](https://github.com/iotaledger/iri/tree/dev/src/main/java/com/iota/iri/zmq).
